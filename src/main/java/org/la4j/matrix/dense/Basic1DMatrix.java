...
public class Basic1DMatrix extends DenseMatrix {
	...
	/**
	 * Creates a zero {@link Basic1DMatrix} of the given shape: {@code rows} x
	 * {@code columns}.
	 */
	...
	/**
	 * Creates a constant {@link Basic1DMatrix} of the given shape and
	 * {@code value}.
	 */
	...
	/**
	 * Creates an unit {@link Basic1DMatrix} of the given shape: {@code rows} x
	 * {@code columns}.
	 */
	...
	/**
	 * Creates a random {@link Basic1DMatrix} of the given shape: {@code rows} x
	 * {@code columns}.
	 */
	...
	/**
	 * Creates a random symmetric {@link Basic1DMatrix} of the given
	 * {@code size}.
	 */
	...
	/**
	 * Creates a {@link Basic1DMatrix} of the given 1D {@code array} w/o copying
	 * the underlying array.
	 */
	public static Basic1DMatrix from1DArray(int rows, int columns,
			double[] array) {
		return new Basic1DMatrix(rows, columns, array);
	}
	...
	public static Basic1DMatrix block(Matrix a, Matrix b, Matrix c, Matrix d) {
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < columns; j++) {
				...
				if ((i < a.rows()) && (j >= a.columns())) {
					array[i * rows + j] = b.get(i, j - a.columns());
				}
				if ((i >= a.rows()) && (j < a.columns())) {
					array[i * rows + j] = c.get(i - a.rows(), j);
				}
				if ((i >= a.rows()) && (j >= a.columns())) {
					array[i * rows + j] = d.get(i - a.rows(), j - a.columns());
				}
			}
		}
	 ...
	 *
	 * @param csv
	 *            the CSV string representing a matrix
	 *
